...
export const IndiaMap: React.FC<IndiaMapProps> = ({
  ...
  // Move the map layer directly on the compositor during gestures. React only
  // receives the settled position, avoiding a full GeoJSON SVG render per event.
  ...
  const beginMapGesture = () => {
    mapLayerRef.current?.style.setProperty('transition', 'none');
  };

  const endMapGesture = () => {
    mapLayerRef.current?.style.removeProperty('transition');
    commitMapTransform();
  };
  ...
  // Real GeoJSON state paths computed strictly via D3 geoPath
  ...
  const handleZoomIn = () => {
    const nextZoom = Math.min(zoomRef.current + 0.35, 2.8);
  ...
  const handleZoomOut = () => {
    queueMapTransform(Math.max(zoomRef.current - 0.25, 0.75), panRef.current);
    commitMapTransform();
  };
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const nextZoom = Math.max(0.75, Math.min(2.8, currentZoom * zoomFactor));
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    if (e.touches.length === 1 && !transitioningCity) {
      isDragging.current = true;
      beginMapGesture();
      dragDistance.current = 0;
      dragStart.current = {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (!isDragging.current || e.touches.length !== 1 || transitioningCity) return;
  ...
  return (
    <div 
      ref={containerRef}
      className="relative isolate h-full w-full overflow-hidden bg-transparent select-none flex flex-col justify-between"
    ...
    >
      ...
      <div className="absolute right-3 top-3 sm:right-4 sm:top-4 z-20 hidden lg:block w-64 xl:w-72 pointer-events-auto">
        <div className="relative rounded-2xl border border-amber-500/25 bg-[#1a120e]/85 p-1 shadow-2xl backdrop-blur-xl ring-1 ring-white/5">
      ...
      <div 
        className="relative mx-auto flex h-full w-full max-w-full items-center justify-center px-2 pt-25 pb-2 sm:px-4 sm:pt-4 md:pt-5 cursor-grab active:cursor-grabbing overflow-hidden"
      ...
      >
        ...
        >
          <svg
            ...
            className="h-full w-full lg:w-auto lg:min-w-[980px] xl:min-w-[1100px] 2xl:min-w-[1220px] select-none filter drop-shadow-[0_0_40px_rgba(232,160,91,0.16)]"
                  ...
                  >
                    ...
                    {(isHovered || isSelected || isConnected) && !isTransitioning && (
                      <circle
                        ...
                        className={isHovered ? 'animate-map-ping' : ''}
                        style={{ transformOrigin: `${cx}px ${cy}px` }}
                      />
                    )}
                    ...
                    <circle
                      ...
                      filter={isTransitioning ? 'url(#transitionGlow)' : 'url(#markerGlow)'}
                    />
                    ...
                    <text
                      ...
                      letterSpacing="0.015em"
                      fontFamily="'Outfit', sans-serif"
                      className="cursor-pointer drop-shadow-[0_2px_4px_rgba(0,0,0,0.95)] select-none transition-all hover:fill-amber-300"
                    >
                      {city.name}
                    </text>
                  </g>
      ...
      <AnimatePresence>
        {transitioningCity && (
          <motion.div
            ...
            className="pointer-events-none absolute top-20 left-1/2 -translate-x-1/2 z-40 flex items-center gap-3 rounded-2xl border border-amber-500/60 bg-[#1a120e]/95 px-5 py-3 shadow-2xl shadow-amber-500/20 backdrop-blur-2xl"
          >
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col gap-2 p-2.5 sm:p-4 pointer-events-none">
        ...
        <div className="pointer-events-auto flex flex-wrap items-stretch gap-2 sm:gap-2.5">
          <div className="flex items-center gap-2 rounded-2xl border border-stone-900/80 bg-[#1a120e]/90 px-3 py-2 sm:px-4 sm:py-2.5 shadow-2xl backdrop-blur-xl ring-1 ring-white/5">
        ...
        <div className="pointer-events-auto flex items-center gap-1 rounded-2xl border border-stone-900/80 bg-[#1a120e]/90 p-1 sm:p-1.5 shadow-2xl backdrop-blur-xl ring-1 ring-white/5">
          <button
            onClick={handleZoomIn}
            className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-xl text-stone-300 hover:bg-stone-900/60 hover:text-amber-300 transition-colors cursor-pointer"
            title="Zoom In"
          >
        ...
        <div className="pointer-events-auto flex items-center gap-2.5 sm:gap-3 rounded-2xl border border-stone-900/80 bg-[#1a120e]/90 px-2.5 sm:px-3 py-1.5 sm:py-2 text-xs text-stone-300 shadow-2xl backdrop-blur-xl ring-1 ring-white/5">
