...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-[#3a2117]/60 bg-[#0f0b09] md:bg-[#0f0b09]/90 md:backdrop-blur-xl">
      {/* LAG FIX: solid navbar on phones. backdrop-blur over the map is very expensive. */}
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* LAG FIX: dropdown uses a solid fill on phones (no backdrop-blur). */}
          {isDropdownOpen && selectionList.length > 0 && (
            <div
              className="absolute right-4 top-full mt-2 w-64 rounded-2xl border border-stone-800 bg-stone-900 p-2 shadow-2xl md:bg-stone-900/95 md:backdrop-blur-xl ring-1 ring-white/10 z-50"
              onMouseLeave={() => {
      ...
      {/* Mobile menu panel */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-[#3a2117]/60 bg-[#0f0b09] px-5 py-4 shadow-2xl">
          <button
            onClick={() => {
              onSelectCity(null);
